...
% Pre-compute PoE ingredients (Task 3 requirement)
poeData = helperComputePoEData(robot_struct, endEffector);
...
[~, Qtheta] = stompTrajCost(robot_struct, theta, R, voxel_world, poeData);
QthetaOld = 0;
...
while abs(Qtheta - QthetaOld) > convergenceThreshold
    ...
    tic
    %% Execute the STOMP update following the provided helper functions
    %% Sample noisy trajectories drawn around the current mean trajectory
    [theta_samples, em] = stompSamples(nPaths, Rinv, theta);

    %% Calculate local trajectory cost for each sampled trajectory
    % variable declaration (holder for the cost):
    Stheta = zeros(nPaths, nDiscretize);
    for k = 1:nPaths
        [Stheta(k,:), ~] = stompTrajCost(robot_struct, theta_samples{k}, R, voxel_world, poeData);
    end
    ...
    %% Compute delta theta (aka gradient estimator, the improvement of the delta)
...
v = VideoWriter('Iiwa14_Training.avi');
v.FrameRate = 15;
open(v);
...
enableVideo = 0;
if enableVideo == 1
    v = VideoWriter('Iiwa14_wEEConY3.avi');
    v.FrameRate =2;
    open(v);
